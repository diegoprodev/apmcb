-- Nova overload de record_lending_batch/record_lending_returns com p_shift_id
-- (rastreabilidade cross-turno, mesma migration de schema
-- 20260923022949_cautela_lending_devolucao_rastreavel.sql). Mesmo padrão já
-- usado neste projeto para essas duas funções: nunca DROP a assinatura
-- anterior, sempre uma nova overload com parâmetro adicional DEFAULT NULL —
-- evita quebrar chamadas em trânsito durante o deploy (client antigo ainda
-- rodando contra schema novo por alguns segundos).

CREATE OR REPLACE FUNCTION public.record_lending_batch(
  p_tenant_id uuid, p_master_id uuid, p_military_id uuid, p_reserve_id uuid,
  p_movement_id uuid, p_notes text, p_auth_mode text, p_biometric_proof_id uuid,
  p_items jsonb, p_totp_claim_id uuid DEFAULT NULL::uuid, p_shift_id uuid DEFAULT NULL::uuid
)
...
-- CRÍTICO (achado de review, 2026-09-22): toda função nova ganha EXECUTE
-- para PUBLIC por default no Postgres — SECURITY DEFINER sem este REVOKE
-- fica invocável direto via PostgREST/supabase-js por qualquer client
-- autenticado, bypassando roleGuard/requireActiveShift do BFF inteiramente.
-- Mesmo padrão canônico já usado por toda outra RPC deste projeto (ver
-- 20260916000000_reserve_rpc_guard_wiring_migration_a.sql) — e existe um CI
-- gate dedicado (ci_role_routine_grants() em 20260911131947) que varre
-- pg_proc.proacl procurando exatamente este furo. Aplicado direto em
-- produção antes desta migration ser commitada (a janela real esteve
-- exposta por alguns minutos).
REVOKE ALL ON FUNCTION public.record_lending_batch(
  uuid, uuid, uuid, uuid, uuid, text, text, uuid, jsonb, uuid, uuid
) FROM PUBLIC, anon, authenticated;

CREATE OR REPLACE FUNCTION public.record_lending_returns(
  p_tenant_id uuid, p_actor_id uuid, p_military_id uuid, p_reserve_id uuid,
  p_lending_ids uuid[], p_notes text DEFAULT NULL::text,
  p_biometric_proof_id uuid DEFAULT NULL::uuid, p_operation_id uuid DEFAULT NULL::uuid,
  p_totp_claim_id uuid DEFAULT NULL::uuid, p_shift_id uuid DEFAULT NULL::uuid
)
...
REVOKE ALL ON FUNCTION public.record_lending_returns(
  uuid, uuid, uuid, uuid, uuid[], text, uuid, uuid, uuid, uuid
) FROM PUBLIC, anon, authenticated;
