-- Achado real (continuação da feature de elegibilidade/quantidade para
-- cautela, ver docs/enterprise/specs/cautela-eligibility-quantity-
-- enterprise.md e migration 20260818100000): quantidade_cautela nunca era
-- descontada de quantidade_disponivel — a view material_availability e a
-- RPC record_lending_batch (rota real usada por "Nova Saída") calculavam
-- disponibilidade só como quantidade_total - saídas ativas, sem saber que
-- uma fração do estoque estava "reservada exclusivamente para cautela"
-- (CAU-05). Efeito prático: era possível emprestar via saída diária TODO
-- o quantidade_total, incluindo as unidades reservadas para cautela — o
-- mesmo estoque físico ficava contabilizado duas vezes (reservado para
-- cautela E disponível para saída ao mesmo tempo).
--
-- Fix: subtrai quantidade_cautela de quantidade_disponivel na view (usada
-- por GET /api/arsenal, pelo check de estoque em PATCH /api/ssa/requests/
-- :id/approve, e em qualquer outro consumidor da view) e na RPC
-- record_lending_batch (POST /api/lendings/batch, a rota real da tela
-- "Nova Saída"). A RPC singular record_lending_returns não precisa de
-- ajuste — devolução nunca consome estoque.

-- ── 1. material_availability: mesma definição de 20260629000007, só com
--       quantidade_cautela subtraída de quantidade_disponivel ──────────────
...
AS
 SELECT mt.id,
    ...
    (mt.quantidade_total
      - COALESCE(sum(l.quantidade) FILTER (WHERE l.status_legacy = 'ativo'), 0)::integer
      - mt.quantidade_cautela)::integer AS quantidade_disponivel,
  ...
  GROUP BY mt.id, mt.nome, mt.category_id, mt.categoria, mt.categoria_slug, mt.descricao,
           ...
           mt.vehicle_model, mt.quantidade_total, mt.photo_url, mt.tenant_id, mt.reserve_id,
           mt.ativo, mt.quantidade_cautela, mt.cautela_habilitada;

-- ── 2. record_lending_batch: mesma definição de 20260714000010, só com o
--       check de estoque considerando quantidade_cautela ────────────────
