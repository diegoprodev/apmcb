...
- **`supabase/onprem-bootstrap/000_auth_shim.sql` só cobre as duas primeiras migrations** (`20260611000001_initial_schema.sql` e `20260611000002_rls_policies.sql` — tabela `auth.users(id,email)` mínima + `auth.uid()`). Ele NÃO cobre `auth.jwt()` (12 lugares em migrations posteriores), `auth.role()`, colunas extras de `auth.users` (encrypted_password, email_confirmed_at, created_at, updated_at, last_sign_in_at), triggers em `auth.users`, nem a extensão `pgcrypto` que `seed_dev` usa. Repetir a pasta `supabase/migrations/` inteira contra um Postgres on-prem real vai bater em referências `auth.*` não definidas por este shim — cobertura completa da superfície `auth.*` é trabalho futuro, não coberto por este plano. Não leia a Task 1 deste plano como "bootstrap de schema on-prem: completo".
- **Decisão registrada (achado I-minor da revisão final):** a implementação entregue mantém `profiles.id → auth.users(id)` (via o shim da Task 1) em vez de repontar a FK para `public.usuarios(id)` como a prosa original de MIGRATION_SPEC.md §5.3 sugeria. Isso preserva o invariante da Fase 1 de que `supabase/migrations/` fica idêntico entre ambientes — melhoria deliberada em relação ao texto original do spec, mas que não tinha sido registrada como decisão até agora.
