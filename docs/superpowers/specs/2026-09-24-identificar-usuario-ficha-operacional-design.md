...
**Data:** 2026-09-24 (v6)
**Status:** v6 — em revisão sênior adversarial (meta ≥ 9,5/10). Nenhum código de produção desta spec foi escrito.
...
- **v4 → 8,8/10.** Revisor conferiu contra `main`, o épico e o bridge. Confirmou: nenhum consumidor aceita prova sem `result='success'`; D8 implementável isoladamente (2 chamadas da política); RPC atômica compatível com `biometric_proof_consumptions` (NOT NULL, FKs, gatilho de escopo); CORS aceita `X-Ficha-Token`; épico migra cautela para prova da ponte (`41e7fef`). Achados: **ALTO** — (A1) a F0a-1 por finalidade era contornável: o servidor só exige usuário esperado em `enroll`/`confirm_saida_militar` (`biometric-proof.ts:76-78`) e o bridge faz 1:N sempre que `expected_user_id` vem vazio (`BiometricProcessor.cs:139-144`) — um desafio `open_shift`/`handover_*`/`sign_cautela_militar` sem usuário esperado revelava pessoa de outra reserva; (A2) inventário da V4 incompleto (`POST /lendings`, `/ssa/modo-a`, `/ssa/requests/:id/deliver`; reserva-alvo do turno no `/batch` de cautela). **MÉDIO** — (M1) `/result` ainda vaza `match_score`/`finger_index` e tirar `failure_reason` quebra telas e C7; (M2) militar transferido com material em posse ficaria sem caminho de devolução; (M3) foto ainda por id do cliente. **BAIXO** — identificação por código revela existência de matrícula de outra reserva; comportamento com D8 recusada; RPC sem `REVOKE ... FROM PUBLIC` e sem revalidação no banco; modo matriz na busca; SSE sem `material_items`; rollout (46 dos 50 vínculos `usuario` com situação inativa); ordem de merge com o épico; caminho do token divergente entre seções; linhas citadas. **Todos tratados nesta v5.**
- **v5 → 8,7/10.** Revisor conferiu main, épico e bridge. Confirmou: exigir usuário esperado fora de `identify`/`return` não quebra nenhum chamador (main e épico já enviam; `handover_*`/`sign_saida_armeiro` sem chamador; o bridge não cria desafio); custódia (`lendings.status_legacy='ativo'`, `cautelamentos.status`, `cautelamentos.reserve_id`) bate com o schema; colunas da revalidação existem em `biometric_proofs`. Achados: **ALTO** — (A1) tirar `matched_user` das finalidades 1:1 quebraria o armar por digital (`_form.tsx:137` compara `matched_user.id` com o militar); (A2) `POST /api/totp/identify` (`totp.ts:533-569`) seguia aberta: devolve perfil e empréstimos do tenant, 404 × 422 enumera e o contador de falhas permite a um armeiro de outra reserva bloquear o código de um militar. **MÉDIO** — "pertencimento" × "vínculo operacional" contraditórios (transferido passava na F0a-1 e era barrado na identificação); `failure_kind` sem destino para vários motivos reais do bridge e cadastro lendo o campo errado; `identify`/`return` com usuário esperado de outra reserva virava 1:1 e escapava da F0a-1; faltavam `/cautelamentos/:id/substitute` e o turno no `/ssa/modo-a`; bloquear conta **inativa** é regra nova, não algo que a tela já diz; resto da foto por id na §5.2. **BAIXO** — "mesmo tempo de resposta" falso; convenções da RPC nova; `lendings.reserve_id` já é NOT NULL; enum de situação com 5 valores; 31ª entrada; `reserve_id` no prefill; SSE do dono antigo; caminhos e linhas; ordem épico × F0a; lacunas de teste; bug antigo no épico (`_livro-client.tsx:86`). **Todos tratados nesta v6.**
...
7. **Regras de identidade dos fluxos:**
   ...
   - `lendings.reserve_id` é NOT NULL (`20260915210000:44`): não há empréstimo sem reserva; o tratamento de nulo em `/identify` (`lendings.ts:254`) é resto sem efeito.
   ...
   - Guardas de situação no servidor: `/lendings/batch` barra só impedimento (`lendings.ts:314`); a emissão de cautela (`POST /cautelamentos`, `cautelamentos.ts:~494-500`) não checa situação nem pertencimento; a identificação por código (`totp.ts:20-50`) não checa situação — uma conta **inativa** consegue armar por código. A interface rotula "Inativo — sem acesso ao sistema" (acesso ao sistema, `_militares-table.tsx:236`) e só o impedimento como "armamento bloqueado" (`:237`): **bloquear material para conta inativa seria regra nova** (D9), não cumprimento de regra existente.
   ...
   - Usuário esperado no desafio: só `enroll` e `confirm_saida_militar` o exigem no servidor (`biometric-proof.ts:76-78`, `biometric.ts:63`); o bridge faz 1:N sempre que `expected_user_id` vem vazio (`BiometricProcessor.cs:139-144`). `/result` devolve `match_score` e `finger_index` (`biometric.ts:482-484`) e `failure_reason` (`:481`), usado pela UI (`biometric-capture-dialog.tsx` `friendlyFailure`; `_militares-table.tsx:162`, que hoje mostra o motivo cru, p. ex. "Falha de captura (código 263)"). O armar por digital confere `result.matched_user.id === militar.id` (`reserva/saidas/nova/_form.tsx:137`). Motivos de falha gerados hoje: bridge — "nenhum candidato bateu", "captura falhou", "tenant key indisponível" (`BiometricProcessor.cs:163,127,135`), "Dedo falso suspeito (LFD)", "Timeout aguardando dedo no leitor", "Captura cancelada", "Falha de captura (código N)", "Falha ao extrair o template capturado", "Nenhum leitor NITGEN detectado", "Falha ao abrir o leitor (código N)" (`NitgenSdkAdapter.cs:43-147`); servidor/simulador aceitam `result='error'` e texto livre (`biometric.ts:94-95`, `biometric-simulator.ts:31-32`).
   - Todos os caminhos que entregam material à pessoa: `/lendings/batch`, `POST /lendings` (`lendings.ts:510-524`, só barra impedimento), `/ssa/modo-a` (`ssa.ts:1084-1090`, por código), `/ssa/requests/:id/deliver` (`ssa.ts:884`; situação checada só na criação do pedido, `:265-280`), emissão de cautela individual e em lote (`cautelamentos.ts:~494-500`, `:596` — esta sem reserva-alvo no guarda de turno), substituição de item cautelado (`POST /cautelamentos/:id/substitute`, entrega a `antiga.militar_id` em `:1575,1595`, turno sem reserva-alvo em `:1467`). `/ssa/modo-a` cria empréstimos ativos (`ssa.ts:1246-1262`) sem `requireActiveShift` (só em `:598/712/896`) e busca o perfil sem filtro de tenant (`:1082-1086`).
   - Remoção de vínculo com a reserva não checa custódia (`profiles.ts:564-583`): um militar transferido pode seguir com material em posse na reserva antiga.
   - Foto: `GET /api/profiles/:id/photo-url` filtra só por tenant (`profiles.ts:815-821`).
   - Identificação por código: `/lendings/identify` (modo TOTP) e `POST /api/totp/identify` (`totp.ts:533-569`, sem consumidor no web nem no bridge) usam `checkTotpForMatricula`, devolvem perfil sem checar reserva (a segunda também os empréstimos ativos do tenant, `:556-562`), respondem 422 × 404 conforme a matrícula exista (`:35-60`) e somam falhas por usuário (`:62-67`) — um armeiro de outra reserva consegue bloquear o código de um militar.
   ...
   - `cautelamentos.status ∈ {ativa, devolvida, substituida, em_revisao, cancelada}`; `cautelamentos.reserve_id` aceita nulo.
   - Épico (não mergeado): `_livro-client.tsx:86` inicia o usuário atual com `useState("")`; `?? null` não pega string vazia, então uma captura iniciada antes do carregamento envia `expected_user_id:""` e recebe 400.
...
9. **Vazamentos entre reservas que já existem (pré-requisito desta feature):**
   ...
   - **V4 — guardas ausentes no servidor** (§1.7): emissão e substituição de cautela, `/ssa/modo-a` e entrega de SSA ignoram impedimento e/ou pertencimento; `/ssa/modo-a` não exige turno. (Bloqueio de conta inativa não existe em nenhum caminho — regra nova, D9.)
...
**Fora:** reescrever saída/devolução/cautela; novas regras de negócio (a ficha só **lê** estados existentes; a F0a-5 apenas faz o servidor cumprir o que o sistema já define e exibe — impedimento e pertencimento à reserva; bloquear conta inativa é regra nova e só entra se D9 aprovar); registro de ocorrência por staff (N7); exportação/impressão; edição de cadastro na ficha; notificações de terceiros; perfil `auditor` (decisão D3).
...
Sem ids/percentuais; fases rotativas após 6 s; sucesso animado; negado chacoalha. Mudança nesta spec: em **todas** as finalidades o navegador recebe `failure_kind` saneado em vez de `failure_reason` cru (§5.0-1c): **não encontrada** (não reconhecida **ou** de outra reserva — indistinguíveis), **dedo** (leitura ruim ou suspeita de dedo falso: "Limpe o dedo e o leitor e tente de novo"), **leitor** (sem contato/desconectado/falha interna do leitor), **tempo** (esgotado/cancelado), **restrita** (só se D8 for recusada), **falha** (qualquer outro caso: "Não foi possível concluir. Tente de novo."). Sem `match_score` nem dedo em falha. Em sucesso, a pessoa reconhecida continua indo ao navegador (o armar depende disso, §5.0-1c).
...
| Pessoa de outra reserva | Mesmo texto que "não encontrada" (sem revelar existência). O tempo **não** é igualado: sem candidato o bridge sincroniza e tenta de novo (`BiometricProcessor.cs:148-159`), então "outra reserva" responde mais rápido. Aceito: distinguir pelo tempo exige o dedo da pessoa no leitor, na frente do armeiro, e não revela nome nem dado |
| Sem digital cadastrada | Ficha via busca; selo "Confirmação: só código" |
| Impedimento / inativo | Impedimento: alerta vermelho, Armar/Cautelar bloqueados, Devolver liberado. Inativo: alerta âmbar "Conta inativa", ações liberadas (bloqueio só se D9 aprovar). Com D8 aprovada, é identificado pela digital; com D8 recusada, a digital responde "Não foi possível confirmar esta pessoa pela digital. Use a busca ou o código dinâmico." (`failure_kind: restrita`, sem nome) e a ficha chega pela busca |
| Leitor sem contato durante a captura | "O leitor não respondeu. Confira se ele está conectado e tente de novo." (`failure_kind: leitor`) |
| Leitura ruim / suspeita de dedo falso | "Não foi possível ler a digital. Limpe o dedo e o leitor e tente de novo." (`failure_kind: dedo`) |
| Militar transferido com material em posse | Encontrado e identificado pela **custódia ativa** na reserva (§5.2); ficha mostra "Não pertence mais a esta reserva — material pendente de devolução"; só Devolver liberado |
...
1. **V1 — checagem na entrada da prova, por 1:N (não por finalidade).**
   - **1a — critério:** em `POST /api/biometric-bridge/challenges/:id/proof` (`biometric-bridge.ts:~405`), `POST /api/biometric/challenges/:id/submit` (`biometric.ts:~556`) e no simulador (`biometric-simulator.ts:~239`), quando `challenge.expected_user_id IS NULL` (identificação 1:N — pela 1b, só `identify` e `return`), a ordem é: (1) **vínculo operacional** da pessoa com `challenge.reserve_id`, pelo helper único `assertOperationalLink` (§5.0-4); sem vínculo → prova gravada `result='failure'`, `failure_reason='fora_da_reserva'`, **mantendo o `matched_user_id` assinado** como evidência (`biometric-proof.ts:97-112`; `result` não é assinado); (2) política biométrica (`assertBiometricPolicy`, D8); recusa → `result='failure'`, `failure_reason='restrita'` (hoje responde 400 ao bridge e o desafio fica pendente até "Tempo esgotado"); (3) sucesso. Com usuário esperado (1:1), o fluxo atual não muda.
   - **1b — usuário esperado obrigatório fora de `identify`/`return`:** `createChallengeSchema` (`biometric.ts:~56`) passa a exigir `expected_user_id` para toda finalidade exceto `identify` e `return` (hoje só `enroll`/`confirm_saida_militar`, `biometric-proof.ts:76-78`). Fecha o desvio de criar um desafio `open_shift`/`handover_*`/`sign_cautela_*`/`sign_saida_armeiro` sem usuário esperado para fazer 1:N. **E o inverso:** `identify` e `return` **recusam** `expected_user_id` (senão um desafio `identify` com o UUID de alguém de outra reserva viraria 1:1 e escaparia da 1a). Na criação, o usuário esperado precisa ser o próprio ator (turno, assinatura do armeiro) ou ter vínculo operacional com a reserva do desafio. Chamadores conferidos pelo revisor da v5: todos já enviam (`_militares-table.tsx:379-380`, `_form.tsx:483-484`, épico `sign-dialog.tsx:193`, `shift-auth-dialog.tsx:157`, `minhas-cautelas:454`); `_desarmamento-modal.tsx:414` (`return`) e o console (`identify`) não enviam; o bridge não cria desafio.
   - **1c — contrato de `GET /challenges/:id/result`** (`biometric.ts:457-484`), igual para todas as finalidades:
     - **sucesso:** `{ result:'success', proof:{ id, finger_index }, matched_user }` — `matched_user` continua saindo (nas 1:1 é o esperado, e `_form.tsx:137` depende dele; nas 1:N só existe depois da 1a). `proof.id` é necessário para `POST /identificacao`; `finger_index` para o cadastro mostrar o dedo. Sem `match_score`.
     - **falha:** `{ result:'failure', failure_kind }`, sem `matched_user`, `match_score`, `finger_index` nem `failure_reason` — o motivo cru **nunca** vai ao navegador (fica em `biometric_proofs` e no log).
     - **`failure_kind`**, calculado no BFF por `lib/biometric-failure-kind.ts` (função pura, testada motivo a motivo com os textos reais da §1.7): `nao_encontrado` ← "nenhum candidato bateu", `fora_da_reserva`; `dedo` ← "Dedo falso suspeito (LFD)", "Falha ao extrair o template capturado"; `leitor` ← "Nenhum leitor NITGEN detectado", "Falha ao abrir o leitor (código N)", "Falha de captura (código N)", "captura falhou", "tenant key indisponível"; `tempo` ← "Timeout aguardando dedo no leitor", "Captura cancelada", desafio expirado; `restrita` ← recusa da política (D8); `falha` ← `result='error'` e **qualquer motivo desconhecido** (padrão seguro). Um código estável enviado pelo bridge no lugar do texto fica como R10.
     - O diálogo (`friendlyFailure`) e o cadastro (`_militares-table.tsx:162`) passam a ler só `failure_kind`, com uma tabela de textos única no web. Regressão obrigatória: armar por digital (`confirm_saida_militar`) continua verificando a pessoa.
   - **1d — identificação por código** (`/lendings/identify`, modo TOTP): ordem obrigatória — (1) resolver a matrícula **no tenant** e checar vínculo operacional com a reserva da sessão, **antes** de ler `totp_secrets` e **antes** de somar falhas; (2) só então conferir o código. Matrícula inexistente, fora da reserva ou código errado respondem **igual** ("Não foi possível confirmar a identidade.", mesmo status), sem nome/foto (`totp.ts:35-60` hoje diferencia 422 × 404). Fora da reserva não soma falha no contador da pessoa (fecha o bloqueio entre reservas). **`POST /api/totp/identify` é removida** (`totp.ts:533-569`; sem consumidor no web nem no bridge).
   - Log `biometric.identify.cross_reserve` / `identify.denied` (sem PII).
...
3. **Busca de pessoas (consolidação, não correção):** endpoint BFF `GET /api/reserva/pessoas?q=` (service role, **só a reserva ativa** — nunca `scopedReserveIds`, que em modo matriz devolve o tenant inteiro, `reserve-scope.ts:45-52`; pessoas com vínculo operacional, mínimo 3 caracteres, `limit 10`, rate limit 20/min com log de negação). `sanitizeSearchTerm` sai de `apps/web/src/lib/search-term.ts:11` para `packages/shared` e é usado nos dois lados. A rota edge `api/admin/search-profiles` não muda (R6).
4. **Helpers únicos** em `lib/reserve-membership.ts` (fim das duas noções soltas):
   - `assertOperationalLink({ tenantId, reserveId, profileId })` — **vínculo operacional** (§5.2), usado por F0a-1, busca, identificação, leitura da ficha e devolução. Chama a função SQL nova `has_operational_link(p_tenant, p_reserve, p_profile)` (STABLE, SECURITY DEFINER, só service role), a **mesma** usada pela RPC `create_ficha_identificacao` — o predicado existe uma vez só.
   - `assertMilitaryEligibleForCustody({ tenantId, reserveId, militaryId })` — **pertencimento** (`reserve_memberships`, substitui `assertMilitaryBelongsToReserve`, `lendings.ts:87-96`) **e** sem impedimento; usado só para liberar Armar/Cautelar (F0a-5).
5. **V4 — guardas no servidor, em todos os caminhos de entrega** (fazem valer o que a interface já diz, §1.7). `assertMilitaryEligibleForCustody` (item 4) aplicado em: `/lendings/batch`, `POST /lendings` (`lendings.ts:510-524`), `/ssa/modo-a` (`ssa.ts:1084-1090`, que também passa a exigir turno ativo e a filtrar o perfil por tenant, `:1082-1086`), `/ssa/requests/:id/deliver` (`ssa.ts:884` — rechecado **na entrega**, não só na criação), emissão de cautela individual e em lote (`cautelamentos.ts:~494-500`, `:596`) e substituição (`POST /cautelamentos/:id/substitute`, `:1575,1595`). O `/batch` e o `substitute` de cautela passam a validar o turno **da reserva-alvo** (`requireActiveShift` com `targetReserveId`; hoje `:596` e `:1467`). A **identificação** permanece neutra e a **devolução** continua permitida em qualquer situação. Negações com log estruturado e mensagem amigável.
   - **Conta inativa não é barrada pela F0a-5.** Barrar seria regra nova (§1.7, D9). Se o dono aprovar a D9, a regra entra no mesmo helper, depois de conferir os dados: hoje 46 dos 50 vínculos `usuario` estão com situação inativa, e aplicar a regra sem essa conferência bloquearia quase todo o efetivo.
...
O token vai no cabeçalho `X-Ficha-Token` (não no caminho — evita o log de acesso, `middleware/access-log.ts:16`, e agrupa a métrica C3 por rota):
- `GET /api/reserva/ficha` — 1ª dobra.
- `GET /api/reserva/ficha/{cautelas|movimentacoes|ocorrencias}?cursor=` — abas, paginadas.
- `GET /api/reserva/ficha/pessoa` — só `{ id, nome_completo, posto, matricula }` para o prefill dos fluxos (§5.4); a reserva é sempre a da sessão.
- `GET /api/reserva/ficha/foto` — URL assinada da foto da pessoa **resolvida pelo token** (não usa `/profiles/:id/photo-url`, que filtra só por tenant, `profiles.ts:815-821`); `no-store`.
A URL da **página** (`/reserva/identificar/f/<token>`) continua com o token (necessário para recarregar), coberto por `no-store`, `no-referrer` e pela amarração à sessão. O SSE (`EventSource` não envia cabeçalho) usa `?t=`; o log de acesso já registra só o caminho, sem query (`middleware/access-log.ts:16`), e passa a registrar também `routePath` (`hono/route`) para a métrica C3. CORS: `maxAge` no preflight, porque o cabeçalho customizado gera preflight em cada GET.

#### 5.1.1 Contrato da 1ª dobra (Zod em `packages/shared`)
```
{
  pessoa: { nome_completo, posto, matricula, tem_foto: boolean,
            situacao: "ativo"|"pendente"|"pendente_biometria"|"inativo"|"impedimento_administrativo",   // os 5 de BiometricSubjectStatus (biometric-policy.ts:3)
...
|---|---|---|---|
| Impedimento administrativo | `profiles.registration_status` | crítica | bloqueia Armar, Cautelar (servidor: `/batch` hoje; demais caminhos a partir da F0a-5) |
| Conta inativa | `registration_status='inactive'` | alta | só avisa; bloqueia Armar/Cautelar apenas se D9 aprovar (regra entra primeiro no servidor) |
| Não pertence mais à reserva (só custódia) | vínculo operacional sem `reserve_memberships` | alta | libera só Devolver |
...
Pendências de turno (`service_log_events.is_pending`) são por turno, não por pessoa: fora. **"Em posse" usa a regra do RPC de devolução** (itens `status_legacy='ativo'` com `reserve_id = reserva da sessão`; a coluna é NOT NULL). As regras ficam em `lib/ficha-pendencias.ts` (função pura, testada tabela a tabela).
...
- **Vínculo operacional** = pertencimento (`reserve_memberships`) **ou** custódia ativa na reserva: empréstimo `status_legacy='ativo'` com `reserve_id` da reserva, ou cautela com `status in ('ativa','em_revisao')` e `reserve_id` da reserva (cautela com `reserve_id` nulo não cria vínculo). Implementado **uma vez** em `has_operational_link` (§5.0-4) e usado em F0a-1, busca, identificação (digital e busca), leitura da ficha, devolução e RPC. É o que mantém um militar transferido com material em posse capaz de devolver. **Pertencimento** sozinho só decide Armar/Cautelar (`assertMilitaryEligibleForCustody`); com custódia mas sem pertencimento, a ficha só libera Devolver.
- **Vínculo verificado antes de qualquer outra consulta**, por `assertOperationalLink` (§5.0-4). Falha → 404 genérico, sem corpo útil; a checagem roda sozinha e primeiro, para que "fora da reserva" e "inexistente" tenham o mesmo custo (anti-enumeração por tempo).
- Movimentações = `lendings` por `military_id` na reserva (não `GET /api/saidas`, que perde lotes).
- Foto: só por `GET /api/reserva/ficha/foto` (§5.1); a ficha não entrega `profile_id` ao navegador.

### 5.3 Identificação na sessão (substitui o `?p=` da v1)
- **`POST /api/reserva/identificacao`**
  - Corpo por digital: `{ via: "biometria", proof_id }`. Validação: `purpose='identify'`, `result='success'`, `actor_id = chamador`, `reserve_id = reserva da sessão`, `matched_user_id` com vínculo operacional com a reserva, frescor ≤ **2 min** (mesmo `DEFAULT_PROOF_TTL_MS` do sistema, `biometric-proof-consumption.ts:1`, sem override), e consumo de uso único (abaixo).
  - Corpo por busca: `{ via: "busca", profile_id }` — validação de vínculo operacional (o `profile_id` vem da busca escopada).
  - Efeito: grava no **servidor**, na tabela nova `ficha_identificacoes` (`token_hash` sha-256 do token, `session_id` da sessão iron-session, `actor_id`, `tenant_id`, `reserve_id`, `profile_id`, `via`, `criado_em`, `identificado_ate`, `expira_em`, `auditado_em`) — **nada no cookie** (a sessão já está perto do limite de 4 KB e gravações concorrentes são "última escrita vence", §1.7). `token` aleatório ≥ 128 bits, só o hash é guardado. **Selo "identificado"** vale 10 min (`identificado_ate`, diferente do frescor de 2 min da *prova*); a **entrada** vale até o fim do turno/sessão (`expira_em` = expiração da sessão, máx. 8 h); até 30 entradas vivas por sessão (fila do início do turno); a 31ª **descarta a mais antiga** (sem erro ao armeiro). As duas vias gravam pela mesma RPC, serializada por sessão (`pg_advisory_xact_lock(hashtext(session_id))`), então requisições simultâneas não estouram o limite. RLS sem policy (só service role); limpeza diária por `pg_cron` das expiradas. Logout revoga pelo `session_id` (mesmo gancho de `revoked_sessions`).
  - Consumo da prova **atômico com a criação da entrada**: RPC nova `create_ficha_identificacao(...)` (SECURITY DEFINER, `SET search_path`, `REVOKE ALL ... FROM PUBLIC, anon, authenticated`, só service role — convenção das RPCs de biometria) que, numa transação: `perform assert_actor_in_reserve(...)` (convenção SP8); na via biometria **revalida a prova no banco** (`SELECT ... FOR UPDATE`: `tenant_id`, `reserve_id`, `actor_id`, `result='success'`, `purpose='identify'`, `matched_user_id IS NOT NULL`, liveness `is not distinct from false` recusa — mesma regra das RPCs de saída/devolução desde `20260924001500` —, `created_at > now() - interval '2 minutes'`); nas duas vias `has_operational_link`; aplica o limite de 30; e insere em `biometric_proof_consumptions` (só via biometria) (`operation_type='identificacao_ficha'`, `unique(proof_id)` garante uso único) e em `ficha_identificacoes`. `consumeBiometricProof` isolado (insert solto, `biometric-proof-consumption.ts:98`) **não** é usado aqui.
  - **Leitura pelo token:** exige `token_hash` + `session_id` da sessão atual + `reserve_id` = reserva **ativa atual** (trocar de reserva não muda o `sessionId`, `auth.ts:175`) + `expira_em > now()`, e **revalida o vínculo operacional** da pessoa a cada leitura. Sessão sem `sessionId` (legada, `session.ts:31` opcional) → falha fechado: "Identificação não encontrada — identifique de novo".
...
- Devolução pela ficha usa o vínculo operacional (§5.2): militar transferido com material em posse continua devolvendo.
...
- **Resolução no servidor:** `/reserva/saidas/nova` é server component que lê o Supabase direto (`page.tsx:10`) e `_cautelas-client` é client component; ambos resolvem `?pessoa=` chamando `GET /api/reserva/ficha/pessoa` (cabeçalho `X-Ficha-Token`) no BFF com o cookie da sessão (server component repassa o cookie, padrão de `lib/verified-user.ts:52`; client usa `bffFetch`). Nenhuma resolução de pessoa pelo Supabase direto.

### 5.5 Tempo real (sem vazamento)
- Novo canal SSE **`reserva-pessoa`** (`/api/realtime/stream?channel=reserva-pessoa&t=<ficha_token>`): o handler resolve o token pelo BFF (`ficha_identificacoes` + sessão); `subs()` ganha o `profile_id` resolvido (hoje recebe só `{userId, tenantId, reserveId}`, `realtime.ts:31,163`). Filtro por pessoa na assinatura (`military_id=eq.<id>` / `militar_id=eq.<id>`, mesmo padrão do `efetivo-sync`, `realtime.ts:37-55`) e por reserva **no callback** (`payload.new.reserve_id` onde a tabela tem a coluna; `ocorrencias` e `profiles` não têm — filtro só por pessoa, aceitável porque o evento não carrega dado e a ficha relê pelo endpoint escopado). **DELETE não é assinado** (não é filtrável pela pessoa; devolução e fim de cautela chegam como UPDATE). O evento transmitido é **mínimo** `{ tipo: "ficha.mudou", secao }` — **nenhuma linha, nenhum id**.
- Também assina `material_items` (já publicada) filtrado por `current_holder_user_id=eq.<id>` e por `ocorrencia_usuario_associado_id=eq.<id>`, para as pendências de item cautelado e de ocorrência da reserva (C6). Limite conhecido: o filtro vale para a linha **nova**, então quando o item deixa de ser da pessoa (campo vira nulo) o evento não chega por aqui; a saída do item da posse chega pelo UPDATE de `cautelamentos` (filtrado por `militar_id`) e a ficha também relê ao voltar o foco da janela.
...
|---|---|---|---|
| T1 | IDOR entre reservas (ficha, abas, busca, histórico) | pessoa resolvida pelo token da sessão; vínculo operacional primeiro; F0 corrige V2/V3 | integração 2 reservas × 2 armeiros |
| T2 | Vazamento na identificação (leitor de outra reserva) | F0a-1: prova gravada como falha (id assinado preservado), navegador recebe só `failure_kind`, nunca `failure_reason` | integração + e2e |
...
| T7 | Enumeração (tempo de resposta, busca) | vínculo primeiro; respostas iguais para inexistente/fora; mín. 3 caracteres; 20/min; tempo da digital não igualado (§4.6, exige o dedo presente) | integração |
...
| T14 | Pessoa impedida ou fora da reserva recebendo material por qualquer caminho (inclusive por código e por SSA) | F0a-5: helper único em `/lendings/batch`, `POST /lendings`, `/ssa/modo-a` (+ turno), `/ssa/requests/:id/deliver`, emissão de cautela (individual e lote) e `substitute` (+ turno da reserva-alvo) | integração, um teste por caminho |
| T16 | Desvio da F0a-1 criando desafio de outra finalidade sem usuário esperado | F0a-1b: `expected_user_id` obrigatório fora de `identify`/`return`; critério da F0a-1a é 1:N, não finalidade | integração |
| T17 | Enumeração e bloqueio cruzado pela identificação por código | F0a-1d: vínculo antes do segredo e do contador; respostas iguais; `POST /api/totp/identify` removida | integração (inclui "armeiro da reserva B erra o código de militar da A 5× → código do militar não bloqueia") |
| T18 | `identify`/`return` com usuário esperado de outra reserva (vira 1:1) | F0a-1b: essas finalidades recusam `expected_user_id`; esperado validado na criação | integração |
| T15 | Token usado após trocar de reserva | leitura exige reserva ativa atual e revalida o vínculo operacional | integração |
...
## 7. Testes
- **Unit (BFF):** `ficha-pendencias` (cada linha da tabela 5.1.2, limites, silêncio de vencimento, só custódia libera só Devolver); validação/consumo da prova de identificação (ator, reserva, pessoa, frescor 2 min, resultado, uso único); gestão de tokens (selo 10 min, entrada até fim de sessão, máx. 30); matriz de `acoes`; detector de consulta sem finalidade.
- **Integração (BFF+banco):** V1 na entrada da prova para `identify`/`return` (bridge, rota de submit e simulador → `/result` e `/lendings/identify` sem dados da pessoa nem `failure_reason`; `matched_user_id` assinado preservado; finalidades 1:1 inalteradas); V4 (um teste por caminho de entrega da T14, devolução liberada, turno da reserva-alvo no `/batch` de cautela); F0a-1b (desafio sem usuário esperado recusado fora de `identify`/`return`); contrato do `/result` por finalidade (sem score/dedo em falha 1:N, `proof.id` no sucesso); vínculo operacional (transferido com custódia identificado e devolvendo, e barrado em Armar/Cautelar; cautela `em_revisao` conta como custódia); `failure_kind` (cada motivo real da §1.7 mapeado; motivo desconhecido e `result='error'` → `falha`); `identify`/`return` com usuário esperado recusados; esperado sem vínculo recusado na criação; código dinâmico (ordem vínculo → segredo → contador; bloqueio cruzado impossível; `/api/totp/identify` responde 404); limite de 30 entradas com requisições simultâneas; D8 recusada (`restrita`, sem nome, sem desafio pendurado); RPC `create_ficha_identificacao` (uso único concorrente); token com troca de reserva e sessão legada; V2 por `scopedReserveIds`; V3; IDOR em todos os endpoints; resposta idêntica inexistente × fora da reserva; RPC de devolução recusando pessoa diferente (regressão); tokens: hash, isolamento por sessão, revogação no logout, auditoria 1× concorrente; `auditLog` 1× por token; SSE sem vazamento (assinante da reserva B não recebe evento da A).
- **Web (vitest):** regressão do armar por digital (`_form.tsx:137` com o contrato novo do `/result`); diálogo e cadastro lendo só `failure_kind` (um texto por tipo); faixa de identificação e expiração; modo consulta; estados de erro por seção; barra de ações com motivos; varredura de termos técnicos; limpeza em `pagehide`.
...
- **Ordem:** a **F0a vem primeiro** — V1 é vazamento ativo em produção e não pode esperar o épico. O épico `biometric-unify-ssa` é rebaseado sobre a F0a (conflitos esperados em `biometric.ts` e no diálogo de captura, resolvidos no rebase) e mergeado **antes da F3** (R9). Antes do merge, corrigir no épico `_livro-client.tsx:86` (captura só habilitada com o usuário atual carregado; teste cobrindo).
- **F0a — Vazamentos e guardas existentes (BFF + ajuste mínimo no web):** V1 na entrada da prova (1:N), usuário esperado (1b), contrato do `/result` com `failure_kind` (+ diálogo e cadastro no web), código dinâmico (1d, remoção de `/api/totp/identify`), V2 por `scopedReserveIds`, V4 (todos os caminhos), busca escopada, `sanitizeSearchTerm` em `packages/shared`, helpers e função SQL `has_operational_link`. *Entregável isolado; corrige produção mesmo sem a ficha.*
- **F0b — Sync de templates por reserva:** spec própria (R8); não bloqueia F1–F4.
- **F1 — Identificação e leitura:** migration `ficha_identificacoes` (+ `pg_cron`) e RPC `create_ficha_identificacao`, D8 aplicada se aprovada, `POST /identificacao`, tokens no servidor, `GET /ficha` (+ `/pessoa`, `/foto`, abas) com `X-Ficha-Token`, pendências, auditoria, índices, cabeçalhos de cache. Sem UI nova.
...
| D9 | Conta **inativa** deve ser impedida de receber material? (Hoje "inativo" só significa sem acesso ao sistema; regra nova.) | não nesta feature: a ficha só avisa. Se sim, a regra entra no helper da F0a-5 **depois** de conferir os dados (46 dos 50 vínculos `usuario` estão inativos hoje; lista agregada por reserva, sem PII) | dono do sistema |
...
| R10 | `failure_kind` depende do texto do motivo enviado pelo bridge | mapeamento testado motivo a motivo e padrão seguro `falha`; o bridge passa a enviar código estável em tarefa própria | backlog |
...
- **BFF:** `routes/biometric-bridge.ts`, `routes/biometric.ts`, `routes/biometric-simulator.ts` (V1 na entrada da prova; `/result` sem `failure_reason`), `routes/cautelamentos.ts` (V2; guardas V4 na emissão e no `substitute`; turno da reserva-alvo), `lib/biometric-policy.ts` (D8), `lib/biometric-proof.ts` (usuário esperado obrigatório fora de `identify`/`return` e proibido nelas), `lib/biometric-failure-kind.ts` (novo, puro), `middleware/access-log.ts` (`routePath`), `routes/ssa.ts` (guardas V4 e turno em `modo-a`; guarda em `deliver`), `routes/lendings.ts` (guardas V4 em `POST /lendings` e `/batch`; `/identify` por código com vínculo antes do segredo e respostas iguais), `routes/totp.ts` (remove `POST /identify`; ordem vínculo → segredo → contador), `lib/reserve-membership.ts` (novo: `assertOperationalLink`, `assertMilitaryEligibleForCustody`), `routes/reserva-pessoas.ts` (novo: busca, identificação, ficha, abas, pessoa para prefill), `lib/ficha-pendencias.ts` (novo, puro), `lib/ficha-tokens.ts` (novo), `routes/realtime.ts` (canal `reserva-pessoa`), `index.ts`, testes em `__tests__/`; migrations: `*_has_operational_link.sql` (F0a), `*_ficha_identificacoes.sql` (tabela + RLS + `pg_cron` + RPC `create_ficha_identificacao`), `*_ficha_indices.sql`, `*_realtime_ocorrencias.sql`.
- **Shared:** schemas Zod da ficha e da identificação; `sanitizeSearchTerm`.
- **Web:** `reserva/page.tsx` (cards), `reserva/identificar/page.tsx` e `reserva/identificar/f/[token]/page.tsx` (novos) + `components/reserva/ficha/*`, `reserva/saidas/nova/page.tsx` e `_form.tsx` (`?pessoa=`; regressão do armar por digital), `reserva/cautelas/_cautelas-client.tsx` (`?pessoa=`), `reserva/saidas/_desarmamento-modal.tsx` (avisar pessoa diferente), `components/reserva/ficha/ficha-privacy-veil.tsx` (novo), `reserva/biometria/_biometric-console-client.tsx` (textos + `failure_kind`), `components/biometric/biometric-capture-dialog.tsx` e `reserva/militares/_militares-table.tsx` (`failure_kind` no lugar de `failure_reason`), `components/profile-avatar.tsx` (variante que recebe a URL da ficha).
