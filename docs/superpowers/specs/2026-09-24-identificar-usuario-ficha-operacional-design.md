...
**Data:** 2026-09-24 (v5)
**Status:** v5 — em revisão sênior adversarial (meta ≥ 9,5/10). Nenhum código de produção desta spec foi escrito.
...
7. **Regras de identidade dos fluxos:**
   ...
   - Guardas de situação no servidor: `/lendings/batch` barra só impedimento (`lendings.ts:314`); a emissão de cautela (`POST /cautelamentos`, `cautelamentos.ts:~494-500`) não checa situação nem pertencimento; a identificação por código (`totp.ts:20-50`) não checa situação — uma conta **inativa** consegue armar por código. A interface já rotula "Inativo — sem acesso ao sistema" e "Impedimento Administrativo — armamento bloqueado" (`_militares-table.tsx:219-236`).
   ...
   - Identificação por código: `/lendings/identify` (modo TOTP) devolve nome e foto sem checar reserva, e `totp.ts:35-60` responde 422 × 404 conforme a matrícula exista.
...
Sem ids/percentuais; fases rotativas após 6 s; sucesso animado; negado chacoalha. Mudança nesta spec: nas finalidades 1:N (`identify`, `return`) o navegador recebe `failure_kind` saneado em vez de `failure_reason` cru (§5.0-1b): **identificado** (segue para a ficha, §5.3), **não encontrada** (não reconhecida **ou** de outra reserva — indistinguíveis), **leitor** (sem contato/desconectado), **tempo** (esgotado/cancelado), **restrita** (só se D8 for recusada). Sem `match_score` nem dedo em falha. As demais finalidades não mudam.
...
| Impedimento / inativo | Alerta vermelho; Armar/Cautelar bloqueados; Devolver liberado. Com D8 aprovada, é identificado pela digital; com D8 recusada, a digital responde "Não foi possível confirmar esta pessoa pela digital. Use a busca ou o código dinâmico." (`failure_kind: restrita`, sem nome) e a ficha chega pela busca |
...
1. **V1 — checagem na entrada da prova, por 1:N (não por finalidade).**
   - **1a — critério:** em `POST /api/biometric-bridge/challenges/:id/proof` (`biometric-bridge.ts:~405`), `POST /api/biometric/challenges/:id/submit` (`biometric.ts:~556`) e no simulador (`biometric-simulator.ts:~239`), quando `challenge.expected_user_id IS NULL` (identificação 1:N), a ordem é: (1) **vínculo operacional** da pessoa com `challenge.reserve_id` (§5.2); sem vínculo → prova gravada `result='failure'`, `failure_reason='fora_da_reserva'`, **mantendo o `matched_user_id` assinado** como evidência (`biometric-proof.ts:97-112`; `result` não é assinado); (2) política biométrica (`assertBiometricPolicy`, D8); recusa → `result='failure'`, `failure_reason='restrita'` (hoje responde 400 ao bridge e o desafio fica pendente até "Tempo esgotado"); (3) sucesso. Com usuário esperado (1:1), o fluxo atual não muda.
   - **1b — usuário esperado obrigatório fora de `identify`/`return`:** `createChallengeSchema` (`biometric.ts:~56`) passa a exigir `expected_user_id` para toda finalidade exceto `identify` e `return` (hoje só `enroll`/`confirm_saida_militar`, `biometric-proof.ts:76-78`). Fecha o desvio de criar um desafio `open_shift`/`handover_*`/`sign_cautela_*`/`sign_saida_armeiro` sem usuário esperado para fazer 1:N. A F0a verifica e ajusta todos os chamadores (SignDialog e ShiftAuthDialog já enviam; `handover_*` e `sign_saida_armeiro` conferidos na implementação).
   - **1c — contrato de `GET /challenges/:id/result`** (`biometric.ts:457-484`): `matched_user` só sai com `result='success'` **e** finalidade `identify`/`return` (nas 1:1 a pessoa já é conhecida pelo chamador); em falha 1:N sai `{ result:'failure', failure_kind }` com `failure_kind ∈ { nao_encontrado, leitor, tempo, restrita }` (`fora_da_reserva` e "nenhum candidato" viram `nao_encontrado`), **sem** `match_score`, `finger_index` nem `failure_reason`. Em sucesso mantém `proof.id` (necessário para `POST /identificacao` e para o cadastro mostrar o dedo). O diálogo (`friendlyFailure`) e `_militares-table.tsx:162` passam a ler `failure_kind` (tabela de tradução única no web). Finalidades 1:1 mantêm o contrato atual.
   - **1d — identificação por código** (`/lendings/identify`, modo TOTP): exige vínculo operacional; matrícula inexistente, fora da reserva ou código errado respondem **igual** ("Não foi possível confirmar a identidade.", mesmo status), sem nome/foto (`totp.ts:35-60` hoje diferencia 422 × 404).
...
5. **V4 — guardas de situação no servidor, em todos os caminhos de entrega** (fazem valer o que a interface já rotula, §1.7). Helper único `assertMilitaryEligibleForCustody({ tenantId, reserveId, militaryId })` em `lib/reserve-membership.ts` (pertencimento **e** situação ≠ impedimento/inativo), aplicado em: `/lendings/batch`, `POST /lendings` (`lendings.ts:510-524`), `/ssa/modo-a` (`ssa.ts:1084-1090`), `/ssa/requests/:id/deliver` (`ssa.ts:265-280` — rechecado **na entrega**, não só na criação), emissão de cautela individual e em lote (`cautelamentos.ts:~494-500`, `:596`); o `/batch` de cautela também passa a validar o turno **da reserva-alvo** (`requireActiveShift` com `targetReserveId`). A **identificação** permanece neutra e a **devolução** continua permitida em qualquer situação. Negações com log estruturado e mensagem amigável.
   - **Rollout (D9):** 46 dos 50 vínculos `usuario` em produção estão com situação inativa. Antes do deploy, gerar a lista agregada (quantos por reserva, sem PII no chat) e **confirmar com o dono** se a situação reflete a realidade ou é dado de teste; o deploy da guarda só acontece após essa confirmação.
...
- Também assina `material_items` (já publicada) filtrado por `current_holder_user_id=eq.<id>` e por `ocorrencia_usuario_associado_id=eq.<id>`, para as pendências de item cautelado e de ocorrência da reserva (C6).
...
| T14 | Conta inativa/impedida recebendo material por qualquer caminho (inclusive por código e por SSA) | F0a-5: helper único em `/lendings/batch`, `POST /lendings`, `/ssa/modo-a`, `/ssa/requests/:id/deliver` e emissão de cautela (individual e lote) | integração, um teste por caminho |
...
| T17 | Enumeração pela identificação por código | F0a-1d: respostas iguais para inexistente, fora da reserva e código errado | integração |
...
- **Pré-requisito:** merge do épico `biometric-unify-ssa` (R9) **antes** da F0a — os dois mexem em `biometric.ts` e no diálogo de captura.
...
| D9 | Rollout da F0a-5: 46 dos 50 vínculos `usuario` estão com situação inativa — refletem a realidade? | confirmar antes do deploy da guarda (lista agregada por reserva) | dono do sistema |
