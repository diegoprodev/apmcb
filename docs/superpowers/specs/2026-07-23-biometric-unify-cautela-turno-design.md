...
**Data:** 2026-07-23 (v11)
**Status:** Revisão de spec concluída — 9,5/10 (rodada 12/v11), zero CRÍTICO/ALTO/MÉDIO. Aguardando revisão do dono do sistema antes de avançar para o plano de implementação.
...
- **v9 → 7,0/10.** Revisor confirmou os 3 itens que a v9 alegou corrigir (fio até `onConfirm`, as 3 referências reais a `biometricAvailable`, `handleBioConfirm` removido sem referência morta) genuinamente corretos, verificados linha a linha contra o código real, incluindo o source do Base UI Tabs. Confirmou também a contagem "9 em `biometric.ts`" e os 2 BAIXO da v8 corretamente resolvidos, e que nada do backend (4.0-4.2) regrediu. Mas achou **1 ALTO novo, introduzido pela própria reescrita da 4.4**: `resetState()`, chamada dentro de `handleBiometricResult` logo após `onConfirm` no caminho de sucesso, faz `setAuthTab("totp")` — e como `TabsContent` (Base UI) tem `keepMounted=false` por padrão, isso desmonta de verdade o painel "biometria" no exato momento em que `BiometricCaptureDialog` está mostrando sua própria tela de sucesso (checkmark), antes até da chamada assíncrona de abrir/fechar turno terminar — o usuário veria a confirmação de captura sumir e a aba TOTP vazia piscar, em 100% dos casos de sucesso via biometria, não um edge case. A analogia com `SignDialog` (que não usa `Tabs` do Base UI e nunca reseta estado no sucesso) cobria "chamar `onConfirm` direto" mas não "resetar estado local sem efeito colateral" — aqui há efeito colateral estrutural que não existe lá. Mais **1 BAIXO**: a linha de risco sobre remover `biometricAvailable` (seção 7), editada pela própria v9, continuava dizendo "2 call sites externos passando a prop" quando na realidade (já documentado na seção 1.2 desta mesma spec) nenhum dos 2 call sites passa a prop hoje. Corrigidos nesta v10: `resetState()` removida do caminho de sucesso de `handleBiometricResult` (só `handleCancel()` reseta a aba — não há necessidade de voltar pra "totp" enquanto o dialog ainda está visivelmente aberto aguardando resposta do backend); linha de risco da seção 7 corrigida pra refletir que nenhum dos 2 call sites passa a prop hoje.
- **v10 → 8,5/10.** Revisor confirmou o fix do ALTO da v9 genuinamente correto (validado contra o código-fonte real do Base UI Tabs) e nenhum CRÍTICO/ALTO novo. Mas, indo além do que a v9 tinha verificado, leu também o código-fonte real do Base UI **Dialog** (não só Tabs) pra responder a pergunta "pra onde vai o estado que deixou de ser resetado" — e achou **1 MÉDIO real**: a justificativa da v10 ("resetState() correto pra esse momento é o de handleCancel()") está factualmente errada — `onOpenChange` do Base UI só dispara em resposta a uma interação do próprio Dialog (Escape, clique fora, botão fechar), nunca quando o pai muda o prop `open` controlado de fora; `_livro-client.tsx` fecha o dialog no sucesso chamando `setShowOpenDialog(false)` diretamente, sem passar por `onOpenChange`/`handleCancel()`. Como as 2 instâncias de `ShiftAuthDialog` nunca desmontam, `authTab`/`totpToken` ficariam presos indefinidamente após um sucesso via biometria — não quebra nada (as 2 abas continuam sempre visíveis), e é consistente com o precedente já em produção de `SignDialog` (que também nunca reseta `method`), mas era um comportamento não decidido conscientemente, só uma alegação textual incorreta. Mais 1 BAIXO cosmético: o comentário sobre o caminho de falha descrevia um `if` implícito quando na verdade `onResult` simplesmente nunca é chamado nesse caminho (`biometric-capture-dialog.tsx:150-153` confirma que só o branch de sucesso invoca `onResult`). Corrigidos nesta v11: `useEffect(() => { if (!open) resetState(); }, [open])` novo, reseta a aba de forma uniforme na transição de fechamento (cobre sucesso e cancelamento pelo mesmo caminho, sem reintroduzir o bug da v9 — nesse ponto o dialog já está fechando, não há mais tela de sucesso a proteger); import de `useEffect` adicionado; comentário do caminho de falha reescrito com precisão.
- **v11 → 9,5/10 — meta atingida.** Revisor verificou a premissa central da correção da v11 contra o código-fonte real do Base UI Dialog (`DialogStore.js:41-60`, `useRenderDialogRoot.js:66`, `ReactStore.js:104-117`) e confirmou: `onOpenChange` de fato nunca dispara quando o pai muda `open` de fora (só quando o próprio Dialog decide fechar via `setOpen`) — a v10 estava errada, a v11 está certa. Confirmou também que o `useEffect` novo não reintroduz o bug da v9 (só roda depois que `handleOpenShift`/`handleCloseShift` já recebeu 200 do BFF, ou seja, depois da tela de sucesso já ter sido pintada e vista) e que o primeiro render (`open` já `false`) e a redundância com os resets síncronos existentes (TOTP/cancelar) são inofensivos. Zero CRÍTICO/ALTO/MÉDIO nesta rodada. Achou só **3 BAIXO cosméticos**, nenhum bloqueando critério de sucesso: (1) janela de ~100ms em que o `<Dialog>` interno do `BiometricCaptureDialog` desaparece sem animação enquanto o `ShiftAuthDialog` externo ainda está no meio do próprio fade-out (a tela de sucesso já foi vista por tempo suficiente antes disso — não é o bug da v9 de novo); (2) a spec documenta o import novo de `useEffect` mas não menciona que `BiometricCaptureDialog`/`BiometricResult` também precisam de import em `shift-auth-dialog.tsx` (óbvio, pego na hora pelo `tsc`, mesma lacuna pré-existente na seção 4.3); (3) a frase "variant/shiftId/reserveId/canCapture são props novas" (linha da seção 4.4) omite `currentUserId`, que o snippet ao lado já usa e que a tabela da seção 9 já lista corretamente. Recomendação do revisor: endereçar os 3 BAIXO no plano de implementação, não vale nova rodada de revisão de spec só por isso. **Spec pronta para implementação.**
...
**Correção**: autorização escopada por purpose, verificada dentro do handler (não no `roleGuard`, que é um gate binário demais para essa granularidade). Novo módulo `apps/bff/src/lib/biometric-authorization.ts`, centralizando as duas cópias duplicadas de `actorCanAccessReserve`/`reserveBelongsToTenant` (`biometric.ts:122-148`, `biometric-simulator.ts:55-80` — hoje idênticas, código morto de duplicação) num único lugar (`actorCanAccessReserve` sobrevive, comportamento idêntico — só deixa de estar duplicada; **não** é substituída pelas 2 funções novas abaixo, que servem só os 4 call sites que precisam de exceção pra `usuario` — detalhamento completo de todos os 11 call sites reais logo após "Aplicação nos 4 call sites"):
...
**Correção da revisão v10 (achado ALTO — `resetState()` no sucesso desmonta a própria tela de sucesso que acabou de aparecer)**: `Tabs`/`TabsContent` deste arquivo é Base UI (`@base-ui/react/tabs`), cujo `Panel` tem `keepMounted=false` por padrão (`tabs/panel/TabsPanel.js:37,110-113`) — trocar `authTab` **desmonta de verdade** o painel inativo do DOM, não só esconde via CSS. `onResult` só dispara quando `BiometricCaptureDialog` já está mostrando seu próprio `<Dialog>` interno de sucesso (checkmark, "Usuário identificado" — `biometric-capture-dialog.tsx:150-154`); chamar `resetState()` (que faz `setAuthTab("totp")`) logo em seguida desmontaria esse painel — e o `<Dialog>` de sucesso dentro dele — antes do usuário conseguir vê-lo, e antes da chamada assíncrona de `onConfirm`/`handleOpenShift`/`handleCloseShift` sequer terminar (ela só fecha o `ShiftAuthDialog` inteiro depois de resolver). Isso aconteceria em **100% das aberturas/encerramentos de turno por biometria bem-sucedidos** — o caminho principal que esta seção existe para habilitar, não um edge case. Diferente de `SignDialog` (seção 4.3), que não usa `Tabs` do Base UI pra alternar método (é `useState` + ternário simples, sem desmontagem) e nunca reseta o método escolhido no sucesso — a analogia "mesmo padrão da 4.3" cobre chamar `onConfirm` direto, mas não cobre resetar estado local, que aqui tem um efeito colateral estrutural que não existe lá. Corrigido: **não resetar a aba dentro de `handleBiometricResult`**.

**Correção da revisão v11 (achado MÉDIO — a v10 afirmava que `handleCancel()` cobriria o reset no fechamento, o que é factualmente incorreto)**: `_livro-client.tsx` (`handleOpenShift`/`handleCloseShift`) fecha o dialog no sucesso chamando `setShowOpenDialog(false)`/`setShowCloseDialog(false)` **diretamente** — isso muda o prop `open` de `ShiftAuthDialog`, mas não passa pelo `onOpenChange={(v) => { if (!v) handleCancel(); }}` do Base UI (`onOpenChange` só dispara em resposta a uma interação do próprio Dialog — Escape, clique fora, botão fechar — nunca como reação a um `open` controlado mudando de fora). Ou seja, `handleCancel()` **nunca roda no caminho de sucesso**, e como as 2 instâncias de `ShiftAuthDialog` (`_livro-client.tsx:504,542`) nunca desmontam (só o prop `open` alterna), `authTab`/`totpToken` ficariam presos no valor de antes do fechamento indefinidamente — na prática, depois de uma abertura de turno bem-sucedida via biometria, a próxima vez que qualquer um dos dois dialogs abrir (mesma sessão de página) começaria direto na aba Biometria, não mais em TOTP. Sem quebrar nada (as 2 abas continuam sempre visíveis, um clique resolve), mas é um comportamento não decidido conscientemente — e é consistente com o precedente já em produção de `SignDialog` (`sign-dialog.tsx:82`, `method` também nunca reseta, nem no sucesso nem no cancelamento). Corrigido com um reset uniforme na transição de fechamento, cobrindo sucesso e cancelamento pelo mesmo caminho, sem reintroduzir o bug da v9 (nesse ponto o dialog já está fechando, não há mais tela de sucesso a proteger):
...
function handleBiometricResult(result: BiometricResult) {
  if (result.proof?.result === "success") {
    ...
    // (Base UI TabsContent, keepMounted=false) a própria tela de sucesso do
    // BiometricCaptureDialog antes do usuário vê-la, e antes de onConfirm
    // (assíncrono) terminar.
  }
  // onResult só dispara no branch de sucesso de BiometricCaptureDialog
  // (biometric-capture-dialog.tsx:150-153) — falha/expirado/cancelamento
  // nunca chamam esta função; o próprio BiometricCaptureDialog mostra seu
  // estado de erro e o botão "Tentar novamente" internamente (mesmo padrão
  // que SignDialog já assume), sem esta função precisar fazer nada
  // (correção da revisão v11, achado BAIXO — precisão do comentário).
}

// Novo (achado MÉDIO da revisão v11): reseta authTab/totpToken quando o
// dialog termina de fechar, por QUALQUER caminho (sucesso via
// handleBiometricResult, TOTP via handleTotpConfirm, ou cancelamento via
// handleCancel — os 3 já chamam resetState() nos seus próprios pontos,
// este efeito é a rede de segurança pro caso que nenhum dos 3 cobria: o
// pai fechando o dialog via `open=false` direto, sem passar por nenhum
// handler local). Roda só na transição open:true→false — não interfere
// com o momento em que BiometricCaptureDialog mostra sua tela de sucesso,
// porque nesse ponto o Dialog já está fechando.
useEffect(() => {
  if (!open) resetState();
}, [open]);
```

`shift-auth-dialog.tsx:3` importa só `{ useState }` de `"react"` — ganha `useEffect` no mesmo import.
...
| Remover `biometricAvailable` de `ShiftAuthDialog` muda a assinatura pública do componente | Só 2 call sites do componente em `_livro-client.tsx` (abrir e encerrar) — **correção da revisão v10 (achado BAIXO)**: nenhum dos dois hoje passa a prop (confirmado via leitura real de `_livro-client.tsx:504-539,542-573` e já documentado na seção 1.2) — "atualizar" aqui significa só confirmar que a remoção não quebra esses 2 call sites, não migrar um valor existente. Distinto das 3 referências *internas* à prop dentro do próprio `shift-auth-dialog.tsx` (linhas 91/129/150), endereçadas na seção 4.4 — achado ALTO da revisão v9. |
...
- [ ] `ShiftAuthDialog` usa `BiometricCaptureDialog` real (purpose `open_shift`/`close_shift`), `biometricAvailable` removido (incluindo as 3 referências internas que dependiam dela — linhas 91/129/150 reais), aba sempre visível; `handleBioConfirm` removido, substituído por `handleBiometricResult` que chama `onConfirm("biometria", undefined, proofId)` diretamente ao suceder, sem resetar a aba nesse ponto (achado ALTO da revisão v9); `useEffect(() => { if (!open) resetState(); }, [open])` novo cobre o reset uniforme no fechamento, por qualquer caminho (achado MÉDIO da revisão v11); footer só mostra botão de confirmação na aba TOTP.
