...
function handleBiometricResult(result: BiometricResult) {
  if (result.proof?.result === "success") {
    ...
    // (assíncrono) terminar. O dialog inteiro fecha quando o caller resolver
    // a requisição — resetState() correto pra esse momento é o de
    // handleCancel(), não daqui.
  }
