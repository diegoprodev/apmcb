...
interface ConfirmarContaFormProps {
  nomeCompleto: string;
  email: string;
  role: string;
}

export function ConfirmarContaForm({ nomeCompleto, email, role }: ConfirmarContaFormProps) {
  ...
  async function handleActivate(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error ?? "Não foi possível ativar sua conta. Tente novamente ou solicite um novo link de ativação.");
      }
      setSuccess(true);
    } catch (err: unknown) {
  ...
  function handleGoToDashboard() {
    // Hard navigation obrigatório — soft navigation (router.replace) deixa o
    // Router Cache do Next reaproveitar payload RSC da sessão anterior quando
    // outro usuário usa a mesma aba em seguida (ver commit 7204251).
    if (role === "admin_global" || role === "superadmin") window.location.href = "/admin";
    else if (role === "armeiro" || role === "admin_reserva") window.location.href = "/reserva";
    else window.location.href = "/efetivo";
  }
