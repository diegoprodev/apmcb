...
export async function POST(req: NextRequest) {
  try {
    ...
    // Descontinuado (spec docs/enterprise/specs/troca-email-acesso-enterprise.md
    // §10) — este branch era um segundo caminho server-side pra "admin muda
    // e-mail de outro usuário", paralelo e mais fraco que o oficial (sem
    // escopo de reserva, sem TOTP do admin, sem duplo opt-in, audit em
    // tabela legada). Nenhuma UI atual chama isto com existing_user_id (o
    // diálogo de edição usa o BFF — enviar-acesso pra 1º acesso, agora
    // /users/:id/email-change pra troca de conta ativa) — mas a rota HTTP em
    // si continuava alcançável direto (achado do pentest
    // cross-tenant-write.pentest.test.ts), sendo uma superfície de bypass
    // real: um caller que soubesse do parâmetro `existing_user_id` escapava
    // do fluxo com TOTP/reserve-scoping/duplo-opt-in indo direto por aqui.
    // Removido em vez de "só corrigido", por SSOT — duas implementações
    // paralelas da mesma ação sensível é o tipo de duplicação que este
    // projeto já penalizou em outros achados de review.
    if (existingUserId) {
      return NextResponse.json(
        {
          error: "Esta operação foi descontinuada. Use \"Convidar\" (primeiro acesso) ou \"Alterar e-mail de acesso\" (conta já ativa) na edição do usuário.",
          code: "DEPRECATED_USE_EMAIL_CHANGE_FLOW",
        },
        { status: 410 }
      );
    }
