/**
 * ML -- Reserva Militares (/reserva/militares)
 *
 * Harness: ML01-ML15
...
const ROUTE = "/reserva/militares";

test.describe("ML -- Reserva Militares", () => {

  test("ML01 -- carga inicial mostra ate 10 militares", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(1000);
    const items = page.locator("tbody tr, [data-testid='militar-card'], [data-testid='militar-row']");
    await items.first().waitFor({ timeout: T.page }).catch(() => {});
    expect(await items.count()).toBeLessThanOrEqual(10);
  });

  test("ML02 -- btn-ver-mais visivel quando ha mais de 10", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(1000);
    const items = page.locator("tbody tr, [data-testid='militar-card']");
  ...
  test("ML03 -- dropdown Ver mais mostra 20 e 30", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(1000);
    const btn = page.getByTestId("btn-ver-mais");
  ...
  test("ML04 -- busca por nome/matricula filtra resultados", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
  ...
  test("ML05 -- botoes toggle card/grade presentes", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
  ...
  test("ML06 -- modo tabela ativa thead", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
  ...
  test("ML07 -- sort por nome inverte na 2a clique", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
  ...
  test("ML08 -- botao Exportar desabilitado sem selecao", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(800);
  ...
  test("ML09 -- checkbox de item ativa Exportar", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(1000);
  ...
  test("ML10 -- pagina carrega sem erro 5xx", async ({ page }) => {
    await login(page, "reserva");
    const res = await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
    expect(res?.status()).toBeLessThan(500);
  });

  test("ML11 -- titulo da pagina visivel", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
    await expect(page.getByRole("heading").first()).toBeVisible({ timeout: T.page });
  });

  test("ML12 -- estado vazio com busca sem resultado", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(800);
    const input = page.locator("input[placeholder*='uscar' i]").first();
  ...
  test("ML13 -- filtro sem-login mostra usuarios sem conta", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}?filter=sem-login`, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(800);
    await expect(page.locator("body")).toBeVisible();
  });

  test("ML14 -- acesso sem autenticacao redireciona", async ({ page }) => {
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
    await page.waitForURL(/\/login/, { timeout: T.page });
    expect(page.url()).toContain("/login");
  });

  test("ML15 -- expandir para 20 -> ate 20 militares", async ({ page }) => {
    await login(page, "reserva");
    await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(1000);
    const btn = page.getByTestId("btn-ver-mais");
