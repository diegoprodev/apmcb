...
test.describe("Login UI session", () => {
  for (const user of ["admin", "reserva", "efetivo"] satisfies UserKey[]) {
    test(`${user}: login por senha cria sessao BFF e nao volta para session_expired`, async ({ page }) => {
      ...
      await page.locator("#email").pressSequentially(expected.email);
      await page.locator("#password").click();
      await page.locator("#password").pressSequentially(expected.password);
      const submit = page.getByRole("button", { name: /^entrar$/i });
      await expect(submit).toBeEnabled();
      await submit.click();

      await page.waitForURL(new RegExp(expected.landAt), { timeout: 20_000 });
